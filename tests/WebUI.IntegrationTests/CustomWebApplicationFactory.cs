...
namespace cleansolution.WebUI.IntegrationTests
{
    public class CustomWebApplicationFactory<TStartup> : WebApplicationFactory<TStartup> where TStartup : class
    {
        ...
        {
            return await GetAuthenticatedClientAsync("jason@clean-architecture", "cleansolution!");
        }
        ...
        private async Task<string> GetAccessTokenAsync(HttpClient client, string userName, string password)
        {
            ...
            var response = await client.RequestPasswordTokenAsync(new PasswordTokenRequest
            {
                ...
                ClientId = "cleansolution.IntegrationTests",
                ClientSecret = "secret",

                Scope = "cleansolution.WebUIAPI openid profile",
                UserName = userName,
                Password = password
            });
